...
class Solution(object):
    def findShortestPath(self, master: 'GridMaster') -> int:
        dirs = [('U', -1, 0, 'D'), ('D', 1, 0, 'U'), ('L', 0, -1, 'R'), ('R', 0, 1, 'L')]
        seen = set()
        ...
        def dfs(i, j):
            ...
            for dir, x, y, inv in dirs:
                if master.canMove(dir):
                    next = (i + x, j + y)
                    if next in seen:
                        cost = master.move(dir)
                        graph[(i, j)].append((*next, cost))
                        master.move(inv)
                    else:
                        seen.add(next)
                        cost = master.move(dir)
        ...
        while pq:
            ...
            if (i, j) == (target[0], target[1]):
                return cost
            for x, y, weight in graph[(i, j)]:
                if (x, y) in dp:
                    if cost + weight < dp[(x, y)]:
                        dp[(x, y)] = cost + weight
                        heappush(pq, (cost + weight, x, y))
                else:
                    dp[(x, y)] = cost + weight
                    heappush(pq, (cost + weight, x, y))
        return -1
