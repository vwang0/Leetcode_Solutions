...
class DiningPhilosophers:
    ...
    def wantsToEat(self,
                   ...
                   putLeftFork: 'Callable[[], None]',
                   putRightFork: 'Callable[[], None]') -> None:
        # a,b: Semaphores for the left and right forks of our philosopher.
        a,b = self.forks[philosopher], self.forks[(philosopher+1)%5]
        if not philosopher:
            # For the philosopher with pid=0, we introduce a "symmetry breaking" condition,
            # where the order for picking up the forks is swapped.
            a,b = b,a
        with a:
            with b:
                pickLeftFork()
